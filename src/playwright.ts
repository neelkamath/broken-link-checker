...
async function test(links: Array<string>): Promise<Record<string, boolean>> {
    ...
    const promises = links.map((link) => testLink(context, link))
    ...
    console.info(`Tested ${links.length} links in ${linksTime} s, averaging ${linksTime / links.length} s per link.`);
    return results;
}

async function testLink(context: BrowserContext, link: string): Promise<boolean> {
    const linkStartTime = Date.now();
    let linkEndTime: number
    let status: number | null = null
    const page = await context.newPage();
    try {
        const response = await page.goto(link, {waitUntil: 'networkidle', timeout: 30_000});
    ...
    console.info(`Tested ${link} with status ${status === null ? 'N/A' : status} in ${(linkEndTime - linkStartTime) / 1_000} s.`);
...
/*
Tested 100 links in 34.287 s, averaging 0.34287 s per link.
100 links seems to be the maximum without hanging.
 */
